...
// delinearizeRGBComponent delinearizes an RGB component, returning a floating-point number.
// 0.0 <= rgbComponent <= 100.0
func delinearizeRGBComponent(rgbComponent float64) float64 {
	return math.Pow((rgbComponent+0.055)/1.055, 2.4) * 100
}
...
// hueOf returns the hue of [linrgb], a linear RGB color, in CAM16, in radians.
func hueOf(linrgb []float64) float64 {
	scaledDiscount := mathUtils.MatrixMultiply(linrgb, scaledDiscountFromLinrgb)
...
// If this possible vertex lies outside of the cube, [-1.0, -1.0, -1.0] is returned.
...
func findResultByJ(hueRadians, chroma, y float64) int {
	j := math.Sqrt(y) * 11.0
	...
	for iterationRound := 0; iterationRound < 5; iterationRound++ {
		...
		linrgb := mathUtils.MatrixMultiply([]float64{rCScaled, gCScaled, bCScaled}, linrgbFromScaledDiscount)
...
// solveToInt finds an sRGB color with the given hue, chroma, and L*, if
// possible.
//
...
func solveToInt(hueDegrees, chroma, lstar float64) int {
	if chroma < 0.0001 || lstar < 0.0001 || lstar > 99.9999 {
		return colorUtils.ArgbFromLstar(lstar)
	}
	hueDegrees = mathUtils.SanitizeDegreesDouble(hueDegrees)
	hueRadians := hueDegrees / 180 * math.Pi
	y := colorUtils.YFromLstar(lstar)
...
// solveToCam finds a CAM16 object with the given hue, chroma, and L*, if
// possible.
//
...
func solveToCam(hueDegrees, chroma, lstar float64) Cam16 {
	return Cam16FromInt(solveToInt(hueDegrees, chroma, lstar))
}
